...
latence = 0.01
permanents, deux, avant, apres = {'rose'}, {
    'rouge', 'gris', 'bleu'}, {'violet', 'marron'}, {'noir', 'blanc'}
...
class joueur:
    ...
    def selectionner(self, t):
        w = demander("Tuiles disponibles : " + str(t) +
                     " choisir entre 0 et " + str(len(t)-1), self)
        ...
        if p.pouvoir and p.couleur in activables:
            ...
            if a:
                ...
                if p.couleur == "blanc":
                    for q in party.personnages:
                        if q.position == p.position and p != q:
                            dispo = {
                                x for x in passages[p.position] if x not in party.bloque or q.position not in party.bloque}
                            w = demander(
                                str(q) + ", positions disponibles : " + str(dispo) + ", choisir la valeur", self)
                            x = int(w) if w.isnumeric() and int(
                                w) in dispo else dispo.pop()
                ...
                if p.couleur == "violet":
                    ...
                    co = demander(
                        "Avec quelle couleur échanger (pas violet!) ?", self)
                ...
                if p.couleur == "gris":
                    ...
                    party.shadow = int(w) if w.isnumeric() and int(
                        w) in range(10) else 0
                ...
                if p.couleur == "bleu":
                    ...
                    w = demander("Quelle sortie ? Chosir parmi : " +
                                 str(passages[x]), self)
                    y = int(w) if w.isnumeric() and int(
                        w) in passages[x] else passages[x].copy().pop()
    ...
    def bouger(self, p, avec, bloque):
        ...
        if p.couleur != 'violet' or p.pouvoir:
            disp = {x for x in pass_act[p.position]
                    if p.position not in bloque or x not in bloque}
            w = demander("positions disponibles : " +
                         str(disp) + ", choisir la valeur", self)
...
class partie:
    def __init__(self, joueurs):
        ...
        self.start, self.end, self.num_tour, self.shadow, x = 4, 22, 1, randrange(
            10), randrange(10)
        ...
        message("!!! Le fantôme est : " +
                self.fantome.couleur, [self.joueurs[1]])
    ...
    def lumiere(self):
        partition = [{p for p in self.personnages if p.position == i}
                     for i in range(10)]
    ...
    def lancer(self):
        while self.start < self.end and len([p for p in self.personnages if p.suspect]) > 1:
            self.tour()
        informer("L'enquêteur a trouvé - c'était " + str(self.fantome)
                 if self.start < self.end else "Le fantôme a gagné")
